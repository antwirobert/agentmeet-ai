...
const pricingCardVariants = cva('rounded-lg p-4 py-6 w-full', {
  variants: {
    variant: {
      ...
      highlighted: 'bg-linear-to-br from-[#0A2340] to-[#050F1C] text-white',
    },
  },
  defaultVariants: {
    variant: 'default',
  ...
  return (
    <div
      className={cn(
        ...
        className,
        'border min-w-0 overflow-hidden'
      )}
    >
      <div className="flex items-end gap-x-4 justify-between min-w-0">
      ...
      <div className="flex flex-col gap-y-2 mt-6">
        ...
        >
          {features.map((feature, index) => (
            <li key={index} className="flex items-start gap-x-2.5 min-w-0">
              <CircleCheckIcon
